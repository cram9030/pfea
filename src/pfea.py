	...
	for beamset,bargs in beam_sets:
		...
		for beam in beamset:
			...
			for i in range(4):
				...
				data[size*dat_dex:size*(dat_dex+1)] = ktmp.data[:]
				row[size*dat_dex:size*(dat_dex+1)] = trow[:]
				col[size*dat_dex:size*(dat_dex+1)] = tcol[:]
				dat_dex = dat_dex+1
	
	print(data[dat_dex*size:(dat_dex+1)*size])
	data = data[:(dat_dex)*size]
	row = row[:(dat_dex)*size]
	col = col[:(dat_dex)*size]
...
def assemble_loads(loads,constraints,tot_dof,length_scaling):
	...
	forces = co.matrix(0.0,(1,tot_dof))#np.zeros(tot_dof)
	dP = co.matrix(0.0,(1,tot_dof))#np.zeros(tot_dof)
...
def equilibrium_error(K,nodemap,F,D,tot_dof,con_dof):
	...
	dF = np.append(np.array(dF),np.zeros(con_dof))
